...
export const metadata = {
  ...
  publishDate: "2025-08-29T20:32:11Z",
  image: '',
  tags: [
...
# 호이스팅(Hoisting)이란?
자바스크립트는 **인터프리터 언어**라서 코드를 위에서부터 한 줄씩 읽고 실행해요.
실제 실행에 들어가기 전, <Tooltip word="인터프리팅" title="인터프리팅">**코드를 한 줄씩 읽고 즉시 실행하는 방식**을 말해요. 실행 전에 전체를 기계어로 변환하는 컴파일과 달리, **읽으면서 곧바로 동작**해요.</Tooltip> 준비 단계에서 엔진은 먼저 **변수/함수 선언을 메모리에 등록**해 둬요.
